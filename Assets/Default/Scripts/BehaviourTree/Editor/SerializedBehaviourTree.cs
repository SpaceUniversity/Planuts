...
namespace Default.Scripts.BehaviourTree {
    ...
    public class SerializedBehaviourTree {
        ...
        public Runtime.BehaviourTree tree;

        // Property names. These correspond to the variable names on the behaviour tree
        const string sPropRootNode = nameof(Runtime.BehaviourTree.rootNode);
        const string sPropNodes = nameof(Runtime.BehaviourTree.nodes);
        const string sPropBlackboard = nameof(Runtime.BehaviourTree.blackboard);
        const string sPropBlackboardKeys = nameof(Runtime.Blackboard.keys);
        ...
        const string sViewTransformPosition = nameof(Runtime.BehaviourTree.viewPosition);
        const string sViewTransformScale = nameof(Runtime.BehaviourTree.viewScale);
        ...
        public Node CloneTree(Node rootNode, Node parentNode, Vector2 position) {
            ...
            Runtime.BehaviourTree.Traverse(rootNode, (n) => {
                sourceNodes.Add(n);
            });
            ...
            // Clone Edges
            foreach (var node in sourceNodes) {
                var children = Runtime.BehaviourTree.GetChildren(node);
